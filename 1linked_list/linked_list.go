...
// 方案：
// 1. 最普通的方法，将K个链表按顺序合并K-1次。设有K个链表，平均每个链表有n个节点，时间复杂度为：
//    (n+n)+(2n+n)+...+((k-1)n+n)=n(2+3+4+...+k)=(k(k+1)/2-1)n，O(k^2*n)
// 2. 将k*n个节点放入数组，再将数组进行排序然后顺序连接。设有K个链表，平均每个链表有n个节点，时间复杂度为：
//    kn*logkn+kn，O(kn*longkn)
// 3. 对K个链表进行分治，两两进行合并。设有K个链表，平均每个链表有n个节点，时间复杂度为：
//    第一轮：进行k/2次，每次处理2n个
//    第二轮：进行K/4次，每次处理4n个
//    .....
